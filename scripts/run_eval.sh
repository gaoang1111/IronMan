...
# RESUME_PATH=${RESUME_PATH:-../checkpoints/phase-cmp-eoc/phase-cmp_step_150}
...
# LR_GENERATOR=${LR_GENERATOR:-1.5e-5}
...
WANDB_PROJECT=${WANDB_PROJECT:-Mark-33-eval}
WANDB_RUN_NAME=${WANDB_RUN_NAME:-Mark-33-eval-phase-full-eoc}
WANDB_RUN_TAGS=${WANDB_RUN_TAGS:-phase3-full-eval,2query,javis,attn,residual,eoc}

RANDOM_GATE=${RANDOM_GATE:-0}
...
if [[ "${PARALLEL}" == "ddp" || "${PARALLEL}" == "fsdp" ]]; then
  torchrun --nproc_per_node "${NPROC_PER_NODE}" -m src.eval "${args[@]}"
else
  python -m src.eval "${args[@]}"
fi
