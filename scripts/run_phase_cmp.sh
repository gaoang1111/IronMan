...
PHASE=${PHASE:-phase-cmp}
DATA_PATH=${DATA_PATH:-/default-vepfs/public/user/ga/Iron/33/data/train.jsonl}
OUTPUT_DIR=${OUTPUT_DIR:-../checkpoints/phase-cmp-denamicq}

# to load phase warmup checkpoint, you need to set RESUME_PATH, LOAD_OPTIMIZER-false, RESET_STEP_ON_RESUME-true
RESUME_PATH=${RESUME_PATH:-../checkpoints/phase-warmup-denamicq/phase1_step_30}
LOAD_OPTIMIZER=${LOAD_OPTIMIZER:-false}
RESET_STEP_ON_RESUME=${RESET_STEP_ON_RESUME:-true}

# to load phase2 checkpoint, you need to set RESUME_PATH, LOAD_OPTIMIZER-true, RESET_STEP_ON_RESUME-false
...
LR=${LR:-3e-5}
LR_PROJECTOR=${LR_PROJECTOR:-2e-5} 
LR_GENERATOR=${LR_GENERATOR:-0}
LR_COMPRESSOR=${LR_COMPRESSOR:-2e-5}
LR_JAVIS_GATE=${LR_JAVIS_GATE:-7e-3}
...
WANDB_PROJECT=${WANDB_PROJECT:-Mark-42-dynamic}
WANDB_RUN_NAME=${WANDB_RUN_NAME:-Mark-42-phase-cmp-dynamicq}
WANDB_RUN_TAGS=${WANDB_RUN_TAGS:-phase-cmp,dynamicq}
