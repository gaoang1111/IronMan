...
class Mark42StreamingEngine:
    def __init__(
        ...
        *,
        ...
        if chunk_stride is None:
            self.chunk_stride = max(1, int(self.chunk_size) // 2)
        else:
    ...
    def stream_generate(
        ...
        *,
        ...
        if self.chunk_stride <= 0:
            raise ValueError(f"chunk_stride must be > 0, got {self.chunk_stride}")
        if self.chunk_stride > self.chunk_size:
            raise ValueError(
                f"chunk_stride must be <= chunk_size, got chunk_stride={self.chunk_stride}, chunk_size={self.chunk_size}"
            )
    ...
    def _calc_cknum(self, raw_len: int) -> int:
        raw_len = int(raw_len)
        if raw_len < int(self.chunk_size):
            return 0
        stride = int(self.chunk_stride)
        full_chunks = 1 + (raw_len - int(self.chunk_size)) // stride
        return max(0, int(full_chunks) - int(self.buffer_num) + 1)
    ...
    def _compress_batch_if_needed(
        ...
        *,
        ...
        stride = int(self.chunk_stride)
        take = int(cknum) * stride
        chunks = [raw_queue[i * stride : i * stride + int(self.chunk_size)] for i in range(int(cknum))]
